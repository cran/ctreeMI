...
report_ctreeMI <- function(object, digits = 3) {
  ...
  txt <- paste0(
    ...
    "of M and correspondingly inflates node-level test statistics, the ",
    "significance threshold used for node selection was divided by M ",
    "(the Stack/M correction; Sherlock et al., 2026), giving a nominal ",
    "alpha of ", format(info$alpha_nominal), " and an applied threshold of ",
    format(signif(info$alpha_applied, digits)), ". The resulting tree ",
  ...
  out <- list(
    ...
    alpha_nominal = info$alpha_nominal,
    alpha_applied = info$alpha_applied,
    n_terminal    = n_term,
    max_depth     = depth,
